...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% OBJECTIVE
%   ===> Perform Monte Carlo simulation for task 2
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% IMPLEMENTATION
...
% Add tau & R
s_tau = (tau-2)/2.8*ones(sample_number,1);   s_R = (R+1)/0.4*ones(sample_number,1);
X = [X(:,1:2),s_tau,X(:,3:4),s_R];
scaled_samples = [scaled_samples(:,1:2),tau/1000*ones(sample_number,1),scaled_samples(:,3:4),R*ones(sample_number,1)];
